...
export const signup = async (req,res) =>{
    ...
    try {
        if(!fullName || !email || !password || !bio){
            return res.json({sucess: false, message: "Missings Details"})
        }
...
export const login = async (req,res) =>{
    try {
        ...
        if(!isPasswordCorrect){
            return res.json({success: false, message: "Invalid credebtials"});
        }
        const token = generateToken(newUser._id)

        res.json({sucess: true, userData, token, message: "Account created Successfully"})
    } catch (error) {
        console.log(error.message)
        res.json({sucess: false, message: error.message})
    }
...
//Cpntroller to update user profile details
